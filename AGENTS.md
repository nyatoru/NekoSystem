...
- **Resource pack zip:** client rejects any zip with directory entries ("Non pack entries found"); Gradle `Zip` writes dir entries for non-empty dirs, so `buildResourcePack` strips them in `doLast` — keep that strip if you touch the task.
