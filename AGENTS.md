...
- **Custom item textures (Hammer/Magnet/UpgradeFurnace/Drawer):** items carry `custom_model_data` (Hammer 1001-1006, Magnet 2001, Furnace 3001-3010, Drawer 4001-4010), NOT `item_model`. The pack overrides vanilla `assets/minecraft/items/<base>.json` with a `select` on `custom_model_data` so players **without the pack see the vanilla item** (pickaxe/compass/furnace/barrel) instead of a broken texture. Furnace uses `super_furnace` block-style models (`nekoplugin:item/super_furnace/sf_<tier>`) under `assets/nekoplugin/models/item/super_furnace/`; Drawer uses `nekoplugin:item/drawers/drawer<tier>`. `isHammer`/`isMagnetItem` still accept the legacy `nekoplugin:hammer_*`/`nekoplugin:magnet` `item_model` fallback; `ensureHammerModel` migrates old stacks. Geyser mappings use `legacy` definitions matching the same CMD values.
