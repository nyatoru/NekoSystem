...
- **Custom item textures (Hammer/Magnet):** items carry `custom_model_data` (Hammer 1001-1006, Magnet 2001), NOT `item_model`. The pack overrides vanilla `assets/minecraft/items/<base>.json` with a `select` on `custom_model_data` so players **without the pack see the vanilla item** (pickaxe/compass) instead of a broken texture. `isHammer`/`isMagnetItem` still accept the legacy `nekoplugin:hammer_*`/`nekoplugin:magnet` `item_model` fallback; `ensureHammerModel` migrates old stacks. Geyser mappings use `legacy` definitions matching the same CMD values.
